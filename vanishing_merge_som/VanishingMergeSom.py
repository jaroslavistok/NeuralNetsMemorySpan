...
class VanishingMergeSom:
    ...
    def train(self, inputs, discrete=True, metric=lambda x, y: 0, alpha_s=0.01, alpha_f=0.001, lambda_s=None,
              lambda_f=1, eps=100, in3d=True, trace=True, trace_interval=10, sliding_window_size=3, log=False, log_file_name=''):
        count = len(inputs)
        if trace:
            ion()
    ...
    def calculate_context(self, beta, history, iteration):
        context = np.array(history[0])
        for i in range(iteration-1):
            context += beta**i * np.array(history[i])
        return context
